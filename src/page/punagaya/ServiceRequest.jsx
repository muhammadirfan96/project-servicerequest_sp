...
import { axiosRT } from "../../../src/config/axios.js";
import { useDispatch, useSelector } from "react-redux";
import { setNotification } from "../../../src/redux/notificationSlice.js";
import { setConfirmation } from "../../../src/redux/confirmationSlice.js";
...
const ServiceRequest = () => {
  ...
  const [searchBased, setSearchBased] = useState("site");

  const findServicerequest = async () => {
    try {
      const response = await axiosInterceptors.get(
        `/${import.meta.env.VITE_APP_NAME}/${
          import.meta.env.VITE_APP_VERSION
        }/service-request?order=desc&limit=${limit}&page=${page}&${key}`,
      );
  ...
  const handleUpdate = async (id) => {
    setForm({ id: id });
    setNamaModal("update service-request");
  ...
  const deleteData = async (id) => {
    try {
      await axiosInterceptors.delete(
        `/${import.meta.env.VITE_APP_NAME}/${import.meta.env.VITE_APP_VERSION}/service-request/${id}`,
      );
      dispatch(
        setNotification({
            ...
            <div className="w-[30%] rounded p-1 shadow shadow-blue-100">
              <p className="border-b border-blue-400 text-xs text-gray-700">
                ...
                >
                  ...
                  <option value="wo_number">wo_number</option>
                  <option value="asset_number">asset_number</option>
                  <option value="equipment_rbd_num">equipment_rbd_num</option>
                  <option value="asset_description">asset_description</option>
                  <option value="location">location</option>
                  <option value="report_by_name">report_by_name</option>
                  <option value="work_group">work_group</option>
                  <option value="shift">shift</option>
                  <option value="site">site</option>
                  <option value="fault_priority">fault_priority</option>
                  <option value="fault_type">fault_type</option>
                  <option value="reported_date">reported_date</option>
                  <option value="needdowntime">needdowntime</option>
                </select>
                <button
              ...
              <table className="min-w-full table-auto text-sm font-light">
                <thead>
                  <tr className="bg-gradient-to-r from-blue-500 to-cyan-500 uppercase text-white">
                    <td className="p-2">service_request</td>
                    <td className="min-w-[250px] p-2 text-left md:min-w-[400px]">
                      summary
                    </td>
                    <td className="p-2">status</td>
                    <td className="p-2">wo_number</td>
                    <td className="p-2">asset_number</td>
                    <td className="p-2">equipment_rbd_num</td>
                    <td className="p-2">asset_description</td>
                    <td className="p-2">location</td>
                    <td className="p-2">report_by_name</td>
                    <td className="p-2">work_group</td>
                    <td className="p-2">shift</td>
                    <td className="p-2">site</td>
                    <td className="p-2">fault_priority</td>
                    <td className="p-2">fault_type</td>
                    <td className="p-2">reported_date</td>
                    <td className="p-2">needdowntime</td>
                ...
                <tbody>
                  {servicerequest.map((each) => (
                    <tr
                      key={each._id}
                      className="border-b border-blue-200 hover:bg-blue-100"
                    >
                      <td className="p-2 text-gray-800">
                        {each.service_request}
                      </td>
                      <td className="p-2 text-gray-800">{each.summary}</td>
                      <td className="p-2 text-gray-800">{each.status}</td>
                      <td className="p-2 text-gray-800">{each.wo_number}</td>
                      <td className="p-2 text-gray-800">{each.asset_number}</td>
                      <td className="p-2 text-gray-800">
                        {each.equipment_rbd_num}
                      </td>
                      <td className="p-2 text-gray-800">
                        {each.asset_description}
                      </td>
                      <td className="p-2 text-gray-800">{each.location}</td>
                      <td className="p-2 text-gray-800">
                        {each.report_by_name}
                      </td>
                      <td className="p-2 text-gray-800">{each.work_group}</td>
                      <td className="p-2 text-gray-800">{each.shift}</td>
                      <td className="p-2 text-gray-800">{each.site}</td>
                      <td className="p-2 text-gray-800">
                        {each.fault_priority}
                      </td>
                      <td className="p-2 text-gray-800">{each.fault_type}</td>
                      <td className="p-2 text-gray-800">
                        {each.reported_date}
                      </td>
                      <td className="p-2 text-gray-800">
                        {each.needdowntime?.toString()}
                      </td>
                      <td className="p-2 text-gray-800">{each.createdAt}</td>
                      <td className="p-2 text-gray-800">{each.updatedAt}</td>
                      <td className="flex p-2">
                        <button
                          disabled={each.status !== "NEW"}
